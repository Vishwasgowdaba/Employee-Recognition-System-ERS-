...
export default function Leaderboard() {
  ...
  const sorted = [...employees].sort((a, b) => b.totalPoints - a.totalPoints);
          ...
          return (
            <Card key={emp.id} className={`glass-card text-center ${isFirst ? "pb-2" : ""}`}>
              <CardContent className={`p-4 ${isFirst ? "pt-6" : "pt-4"}`}>
                ...
                <Badge className="mt-2 gradient-gold text-gold-foreground border-0">{emp.totalPoints.toLocaleString()} pts</Badge>
              </CardContent>
            </Card>
          );
        })}
          ...
          {sorted.map((emp, i) => (
            <div key={emp.id} className="flex items-center gap-4 p-4 border-b border-border last:border-b-0 hover:bg-muted/50 transition-colors">
              ...
              <div className="text-right">
                <p className="font-bold text-gold">{emp.totalPoints.toLocaleString()}</p>
